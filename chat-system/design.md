...
│  4. LLM 调用 (DeepSeek)                         │
│  5. 响应格式化                                   │
│  6. 存储 AI 响应 (messages) ⭐ N8N 内存储       │
│  7. 检查是否触发图片生成                         │
└──────┬──────────────────────────────────────────┘
       │ Response
       │ {response, intentType, mode, metadata}
       ↓
┌─────────────────────────────────────────────────┐
│         Supabase Edge Function                  │
│              (chat-handler)                     │
│                                                 │
│  8. 提取并存储记忆 (character_user_memories)    │
│  9. 更新会话时间戳 (conversations)               │
│  10. 返回响应给客户端                            │
└─────────────────────────────────────────────────┘
```
...
**关键变化**：
1. **用户消息提前存储**：在调用 N8N 之前，Edge Function 就将用户消息存储到数据库，确保消息不丢失
2. **AI 响应在 N8N 中存储**：N8N 工作流在生成响应后立即存储到数据库，减少 Edge Function 的职责
3. **图片生成集成**：N8N 工作流中增加了图片生成的触发检查和处理逻辑

**优势**：
- ✅ 用户消息立即持久化，即使 N8N 调用失败也不会丢失
- ✅ AI 响应存储更及时，减少网络往返
- ✅ Edge Function 职责更清晰，专注于上下文聚合和记忆管理
- ✅ 支持图片生成等扩展功能

### 数据流向

1. **请求阶段**: 客户端 → Edge Function → 数据库（读取上下文 + 写入用户消息）
2. **AI 处理阶段**: Edge Function → N8N → LLM → 数据库（写入 AI 响应）
3. **响应阶段**: N8N → Edge Function → 数据库（写入记忆）→ 客户端

**存储时序**：
```
时间线：
T1: Edge Function 存储用户消息
T2: N8N 调用 LLM 生成响应
T3: N8N 存储 AI 响应到数据库
T4: N8N 返回响应给 Edge Function
T5: Edge Function 提取并存储记忆
T6: Edge Function 返回响应给客户端
```
...
**职责**:
- AI 意图识别
- 对话模式选择（苏格拉底式、情感支持、正常对话、图片生成）
- Prompt 构建和 LLM 调用
- AI 响应存储到数据库
- 响应格式化和返回
- 图片生成触发和处理

### 3. N8N Workflow Context Interface
...
interface N8NResponse {
  ...
  mode: string;           // 使用的对话模式
  metadata?: {            // 元数据（可选）
    image_theme?: string;           // 图片主题
    image_style?: string;           // 图片风格
    image_emotion?: string;         // 图片情感
    trigger_generation?: boolean;   // 是否触发图片生成
    generationData?: {              // 图片生成数据
      prompt: string;
      theme: string;
      style: string;
      emotion: string;
...
**注意**：N8N 工作流现在负责存储 AI 响应到数据库，使用 Supabase 节点直接写入 `messages` 表。
...
**saveUserMessage**:
```typescript
async function saveUserMessage(message: MessageInput): Promise<string> {
  // 在 Edge Function 中调用
  // 插入用户消息到 messages 表
  // 返回消息 ID
}
...
//   - character_id: {{ $('Webhook').item.json.body.character.id }}
//   - role: 'assistant'
//   - content: {{ $json.response }}
//   - intent_type: {{ $json.intentType }}
//   - mode: {{ $json.mode }}
...
### Property 2: 消息存储完整性
*对于任何*成功的聊天请求，用户消息应该在调用 N8N 之前被存储到 messages 表中，AI 响应应该在 N8N 工作流中被存储，且包含完整的元数据（intent_type、mode、metadata），同时会话的 updated_at 时间戳应该被更新。
**Validates: Requirements 3.1, 3.2, 3.3, 3.5**

**存储时序保证**：
- 用户消息在 T1 时刻存储（Edge Function）
- AI 响应在 T3 时刻存储（N8N Workflow）
- 即使 N8N 调用失败，用户消息也已持久化
...
*对于任何*聊天请求，即使记忆提取或会话更新等异步操作失败，系统仍应该返回 AI 响应给客户端，不阻塞主流程。用户消息存储失败会导致请求失败（因为在调用 N8N 之前），但 AI 响应存储失败不会影响返回（因为在 N8N 内部）。
**Validates: Requirements 3.4, 4.5, 8.3, 8.4**

**失败处理**：
- 用户消息存储失败 → 返回 500 错误，不调用 N8N
- N8N 调用失败 → 返回 500 错误，但用户消息已保存
- AI 响应存储失败 → N8N 记录错误，但仍返回响应
- 记忆提取失败 → 记录日志，不影响响应返回
...
*对于任何*从 N8N Workflow 返回的响应，应该包含 response、intentType、mode 字段，可选包含 metadata 和 memories 字段。如果必需字段缺失，Edge Function 应该使用默认值或返回错误。
**Validates: Requirements 2.6**

### Property 13: 图片生成触发
*对于任何*包含图片生成意图的请求，N8N 应该正确识别 trigger_generation 标志，并在满足条件时触发图片生成流程，生成的图片记录应该存储到 user_generated_images 表中。
...
1. **用户消息存储失败**: 返回 500 错误，不调用 N8N（因为消息未持久化）
2. **N8N 调用失败**: 返回友好错误信息，但用户消息已保存
3. **AI 响应存储失败**: N8N 记录错误日志，但仍返回响应给 Edge Function
4. **记忆提取失败**: 记录错误日志，不影响主流程
5. **图片生成失败**: 记录错误日志，不影响对话响应返回
...
1. **增加数据库写入节点**: N8N 负责存储 AI 响应到 Supabase
2. **简化 Prompt 构建**: 使用 Edge Function 传递的完整上下文
3. **保留核心 AI 逻辑**: 意图识别、模式选择、LLM 调用
4. **优化内存管理**: 使用 Simple Memory 节点管理对话上下文
5. **集成图片生成**: 支持图片生成意图识别和触发
6. **响应格式增强**: 返回 metadata 和 memories 字段

**关键节点**：
- `AI Agent`: 意图识别
- `Switch`: 模式路由
- `chat/chat1`: LLM 调用（DeepSeek）
- `响应格式化`: 统一响应格式
- `存储AI响应`: Supabase 写入节点 ⭐ 新增
- `检查是否触发图片生成`: 图片生成判断 ⭐ 新增

### Edge Function 性能优化

1. **并行查询**: 使用 Promise.all 并行查询用户、角色、历史、记忆
2. **连接池**: 复用 Supabase 客户端连接
3. **提前存储**: 用户消息在调用 N8N 前存储，减少等待时间
4. **异步记忆提取**: 记忆提取使用异步方式，不阻塞响应返回
5. **超时控制**: N8N 调用设置 30 秒超时
...
// 调用 N8N（AI 响应在 N8N 中存储）
const aiResponse = await callN8N(context);

// 记忆提取（异步，不阻塞）
extractAndSaveMemories(aiResponse.memories).catch(err => {
